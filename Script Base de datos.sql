...
CREATE TABLE dbo.Ventas (
    ...
    CONSTRAINT UQ_Ventas_pedido UNIQUE      (id_pedido),
...
CREATE TABLE dbo.Bitacora_Acceso (
    ...
    resultado   NVARCHAR(20)   NOT NULL CONSTRAINT DF_BitAcc_resultado DEFAULT 'exitoso',
    ...
    CONSTRAINT CK_BitAcc_evento   CHECK (tipo_evento IN ('LOGIN','LOGOUT','BLOQUEO','SESION_EXPIRADA')),
    CONSTRAINT CK_BitAcc_result   CHECK (resultado   IN ('exitoso','fallido'))
);
GO
...
CREATE INDEX IX_BitPed_pedido        ON dbo.Bitacora_Pedidos      (id_pedido,   fecha_hora DESC);
...
PRINT 'Base de datos COLIBRI v2.0 creada exitosamente.';
GO
